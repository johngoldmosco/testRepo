...
public partial class portal_member_ActiveEpin : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid  WHERE a.status=1 AND a.userid=" + Session["UserID"] + "" + StrSearch + "");

        strQuery = "SELECT a.userid, b.my_sponsar_id AS User_ID, a.ownerid, c.my_sponsar_id AS Owner_ID, a.epin, a.epin_type, d.pin_type, a.epin_cost, DATE_FORMAT(a.created_date,'%Y%m%d') AS CreatedDate, a.status FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid INNER JOIN mlm_epin_type d ON a.epin_type=d.id  WHERE a.status=1 AND a.userid=" + Session["UserID"] + " " + StrSearch + "  ORDER BY a.created_date DESC Limit " + intStart + "," + strpageSize + "";
    ...
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.userid, b.my_sponsar_id, a.ownerid, c.my_sponsar_id, a.epin, a.epin_type, a.epin_cost, DATE_FORMAT(a.created_date,'%Y%m%d') AS CreatedDate FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid  WHERE a.status=1 AND a.userid="+ Session["UserID"] +"  ORDER BY a.created_date DESC";
            ds = clsOdbc.getDataSet(strQuery);
