...
public partial class portal_member_UsedEpin : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid INNER JOIN mlm_login d ON a.used_by=d.userid AND a.status=0  AND a.userid=" + Session["UserID"] + " " + StrSearch + "");

        strQuery = "SELECT a.userid, b.my_sponsar_id AS User_ID, a.ownerid, c.my_sponsar_id As Owner_ID, a.epin, a.epin_type, a.epin_cost, DATE_FORMAT(a.created_date,'%Y%m%d') AS CreatedDate, DATE_FORMAT(a.used_date,'%Y%m%d') AS UsedDate, a.used_by, d.my_sponsar_id As UsedByID, a.status FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid INNER JOIN mlm_login d ON a.used_by=d.userid AND a.status=0 " + StrSearch + " AND a.userid="+ Session["UserID"] +"  Order By a.used_date DESC Limit " + intStart + "," + strpageSize + "";
    ...
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.userid, b.my_sponsar_id AS User_ID, a.ownerid, c.my_sponsar_id As Owner_ID, a.epin, a.epin_type, a.epin_cost, DATE_FORMAT(a.created_date,'%Y%m%d') AS CreatedDate, DATE_FORMAT(a.used_date,'%Y%m%d') AS UsedDate, a.used_by, d.my_sponsar_id As UsedByID, a.status FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid INNER JOIN mlm_login d ON a.used_by=d.userid WHERE a.status=0 AND a.userid=" + Session["UserID"] + "  Order By a.used_date DESC";
            ds = clsOdbc.getDataSet(strQuery);
