...
public partial class portal_member_rptRepurchase : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_repurchase a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_epin_type c ON a.product_id=c.id WHERE a.userid = " + Session["UserID"] + " " + StrSearch + " ");

        strQuery = "SELECT a.id, a.transID, a.userid, b.my_sponsar_id, a.product_id, c.pin_type, a.total_amount, a.total_bv, a.epin, a.gst, a.gross_amount, DATE_FORMAT(a.repurchase_on,'%d-%b-%Y') AS RepurchaseOn, a.total_amount AS Amount  FROM mlm_repurchase a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_epin_type c ON a.product_id=c.id WHERE a.userid= " + Session["UserID"] + " " + StrSearch + " Order By a.repurchase_on DESC";
    ...
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.id, a.userid, b.my_sponsar_id, a.product_id, c.prod_name, a.total_amount, a.total_bv, a.total_tax, a.gross_amount, DATE_FORMAT(a.repurchase_on,'%Y-%m-%d') AS RepurchaseDate FROM mlm_repurchase a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_product c ON a.product_id=c.id WHERE a.userid='" + Session["UserID"] + "'  Order By a.repurchase_on DESC";
            ds = clsOdbc.getDataSet(strQuery);
