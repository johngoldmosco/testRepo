...
public partial class portal_member_rptSponsorIncome : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_sponsor_income a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c ON a.userid=c.userid INNER JOIN mlm_login d ON a.child_id=d.userid INNER JOIN mlm_personal_details e ON a.child_id=e.userid  WHERE a.userid = " + Session["UserID"] + " " + StrSearch + " ");
    ...
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.child_id, d.my_sponsar_id AS CID,e.username AS CNAME, a.userid, b.my_sponsar_id AS UID,c.username AS UNAME, a.total_amount, a.percent, a.sponsor_income, a.tds, a.service_charge, a.net_amount, a._level, a.created_on FROM mlm_sponsor_income a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c ON a.userid=c.userid INNER JOIN mlm_login d ON a.child_id=d.userid INNER JOIN mlm_personal_details e ON a.child_id=e.userid WHERE a.userid ='" + Session["UserID"] + "'  Order By a.created_on DESC";
            ds = clsOdbc.getDataSet(strQuery);
