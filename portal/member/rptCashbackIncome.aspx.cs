...
public partial class portal_member_rptCashbackIncome : System.Web.UI.Page
{
    ...
    protected string Search()
    {
        ...
        {
            strsel = strsel + " AND DATE(a.received_on) BETWEEN '" + txtStartDate.Text + "' AND '" + txtEndDate.Text + "'";
        }
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_cashback_income a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c On a.userid= c.userid WHERE  a.userid = " + Session["UserID"] + " AND a.status=1 " + StrSearch + " ");

        strQuery = "SELECT a.userid, b.my_sponsar_id, c.username, a.amount, a.trans_month, DATE_FORMAT(a.received_on,'%Y-%m-%d') AS Received_on, CASE a.status WHEN  0 THEN 'Pending' WHEN 1 THEN 'Approved' END as status FROM mlm_cashback_income a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c On a.userid= c.userid WHERE  a.userid = " + Session["UserID"] + " AND a.status=1 " + StrSearch + " Order By a.id DESC";
    ...
    {
        Response.Redirect("rptCashbackIncome.aspx");
    }
    protected void lnkbtnExportExcel_Click(object sender, EventArgs e)
    {
        CommonFunctions.exportFile("rptCashbackIncome.xls", ".xls", gvBinaryIncome, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.userid, b.my_sponsar_id, c.username, a.amount, a.trans_month, DATE_FORMAT(a.received_on,'%Y-%m-%d') AS Received_on, CASE a.status WHEN  0 THEN 'Pending' WHEN 1 THEN 'Approved' END as status FROM mlm_cashback_income a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c On a.userid= c.userid WHERE  a.userid = " + Session["UserID"] + " AND a.status=1 Order By a.id DESC";
            ds = clsOdbc.getDataSet(strQuery);
