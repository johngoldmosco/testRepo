...
public partial class portal_member_rptRank : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_login a LEFT JOIN mlm_binary_daily_payout b ON a.userid=b.userid WHERE a.userid='" + Session["UserID"] + "' GROUP BY a.userid  " + StrSearch + " ");

        strQuery = "SELECT a.userid, COALESCE(SUM(b.match_comit),0) AS business, CASE a.rank_id WHEN 0 THEN 'NONE' WHEN 1 THEN 'Silver' WHEN 2 THEN 'Gold' WHEN 3 THEN 'Crown' WHEN 4 THEN 'Platinum' WHEN 5 THEN 'Diamond' WHEN 6 THEN 'Royal Diamond' WHEN 7 THEN 'Double Royal Diamond' WHEN 8 THEN 'Blue Diamond' WHEN 9 THEN 'Red Diamond' END AS Rank , a.my_sponsar_id FROM mlm_login a LEFT JOIN mlm_binary_daily_payout b ON a.userid=b.userid WHERE a.userid='" + Session["UserID"] + "' GROUP BY a.userid " + StrSearch + " ORDER BY `business` DESC" + " ";
