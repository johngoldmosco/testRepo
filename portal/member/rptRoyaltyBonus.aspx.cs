...
public partial class portal_member_rptRoyaltyBonus : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_royalty_income a INNER JOIN mlm_login b ON a.userid=b.userid WHERE a.userid = " + Session["UserID"] + " " + StrSearch + " ");

        strQuery = "SELECT a.userid, b.my_sponsar_id, a.rank, a.amount, a.description, DATE_FORMAT(a.created_on,'%Y-%m-%d') AS BonusDate FROM mlm_royalty_income a INNER JOIN mlm_login b ON a.userid=b.userid WHERE a.userid = " + Session["UserID"] + " " + StrSearch + " Order By a.created_on DESC";
    ...
    {
        CommonFunctions.exportFile("reportRoyalty.xls", ".xls", gvBinaryIncome, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.userid, b.my_sponsar_id, a.product_id, c.prod_name, a.total_amount, a.total_bv, a.total_tax, a.gross_amount, DATE_FORMAT(a.repurchase_on,'%Y-%m-%d') AS RepurchaseDate FROM mlm_repurchase a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_product c ON a.product_id=c.id WHERE a.userid='" + Session["UserID"] + "'  Order By a.created_on DESC";
            ds = clsOdbc.getDataSet(strQuery);
