...
public partial class portal_member_rptRepurchaseLevel : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_repurchase_level_income a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON  a.child_id=c.userid WHERE a.userid = " + Session["UserID"] + " " + StrSearch + " ");

        strQuery = "SELECT a.userid, b.my_sponsar_id AS User_ID, a.child_id, c.my_sponsar_id AS ChildID, a.level, a.total_amount, a.level_percent, a.comm, DATE_FORMAT(a.created_on, '%Y-%m-%d') AS BonusDate FROM mlm_repurchase_level_income a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON  a.child_id=c.userid WHERE a.userid = " + Session["UserID"] + " " + StrSearch + " Order By a.created_on DESC";
    ...
    {
        CommonFunctions.exportFile("reportLevelIncome.xls", ".xls", gvBinaryIncome, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.userid, b.my_sponsar_id AS User_ID, a.child_id, c.my_sponsar_id AS ChildID, a.level, a.total_amount, a.level_percent, a.comm, DATE_FORMAT(a.created_on, '%Y-%m-%d') AS BonusDate FROM mlm_repurchase_level_income a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON  a.child_id=c.userid WHERE a.userid WHERE a.userid='" + Session["UserID"] + "'  Order By a.created_on DESC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    {
        Response.Redirect("rptRepurchaseLevel.aspx");
    }
