...
public partial class portal_admin_OnPageProductManager : System.Web.UI.Page
{
    ...
    public string search_click()
    {
        ...
        if (txtProdName.Text != "")
        {
            strsel = strsel + " AND prod_name  LIKE '%" + Convert.ToString(txtProdName.Text) + "%' ";
        }
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_product WHERE 1 " + strSel + " ");

        strQuery = "SELECT id, prod_name, SUBSTRING(prod_desc,1,20) As prod_desc1, prod_price, photo, CASE status WHEN 1 THEN 'Enabled' WHEN 2 THEN 'Disabled' END AS Status, active FROM `mlm_product` WHERE 1 " + strSel + " Order By id ASC" + " LIMIT " + intStart + "," + strpageSize + "";
    ...
    protected void gvUsers_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
    {
        ...
        foreach (GridViewRow rw in gvUsers.Rows)
        {
            ...
            lnkActivate.NavigateUrl = "UpdateProductOnPage.aspx?ProdID=" + strUserID;
        }
    }
    ...
    {
        CommonFunctions.exportFile("ProductManager.xls", ".xls", gvUsers, pnllead);
    }
    protected void btnAddProduct_Click(object sender, EventArgs e)
    {
        Response.Redirect("AddProductsToOnPage.aspx");
    }
}