...
public partial class portal_admin_rptRepurchaseIncome : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM `mlm_my_repurchase_income` a INNER JOIN mlm_login b ON a.userid=b.userid WHERE 1" + StrSearch + " ");

        strQuery = "SELECT a.id, a.userid,b.my_sponsar_id, a.rep_amt, a.comm, a.percent, DATE_FORMAT(a.created_on,'%Y%b%d') AS CreatedOn,  a.status  FROM `mlm_my_repurchase_income` a INNER JOIN mlm_login b ON a.userid=b.userid WHERE 1 " + StrSearch + " Order By a.created_on DESC";
    ...
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.id, a.userid, a.rep_amt, a.comm, a.percent, DATE_FORMAT(a.created_on,'%Y%b%d') AS CreatedOn, a.status FROM `mlm_my_repurchase_income` a INNER JOIN mlm_login b ON a.userid=b.userid WHERE 1  Order By a.created_on DESC";
            ds = clsOdbc.getDataSet(strQuery);
