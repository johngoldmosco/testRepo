...
public partial class portal_admin_ReviewManager : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM tbl_reviews WHERE 1 " + StrSearch + "");

        strQuery = "SELECT a.`id`, a.`userid`, b.my_sponsar_id, c.username, a.`comment`, CASE a.status WHEN 0 THEN 'Hide' WHEN 1 THEN 'SHOW' END AS Status, DATE_FORMAT(a.created_on, '%d-%b-%y') AS CreatedOn FROM `tbl_reviews` a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c ON a.userid=c.userid WHERE 1 " + StrSearch + " Limit " + intStart + "," + strpageSize + "";
    ...
    {
        Response.Redirect("ReviewManager.aspx");
    }
    protected void lnkbtnExportExcel_Click(object sender, EventArgs e)
    {
        CommonFunctions.exportFile("ReviewManager.xls", ".xls", gvReport, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.`id`, a.`userid`, b.my_sponsar_id, c.username, a.`comment`, CASE a.status WHEN 0 THEN 'Hide' WHEN 1 THEN 'SHOW' END AS Status, DATE_FORMAT(a.created_on, '%d-%b-%y') AS CreatedOn FROM `tbl_reviews` a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c ON a.userid=c.userid WHERE 1 Order By a.created_on DESC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    protected void lnkActive_Click(object sender, System.EventArgs e)
    {
        ...
        foreach (GridViewRow rw in gvReport.Rows)
        {
            ...
            {
                int intCurStatus = clsOdbc.executeScalar_int("SELECT status FROM tbl_reviews WHERE id=" + vid);

                if (intCurStatus != 1)
                {
                    clsOdbc.executeNonQuery("UPDATE tbl_reviews SET status=1 WHERE id=" + vid + "");
                }
                i = i + 1;
            }
        }
    ...
    protected void lnkDisable_Click(object sender, System.EventArgs e)
    {
        ...
        if (gvReport.Rows.Count == 0)
        {
            lblError.Text = "* No records to Hide.";
            return;
        }
        ...
        foreach (GridViewRow rw in gvReport.Rows)
        {
            ...
            if (chkSel.Checked == true)
            {
                ...
                if (intCurStatus != 0)
                {
                    clsOdbc.executeNonQuery("UPDATE tbl_reviews SET status=0 WHERE id=" + vid + "");
                }
                i = i + 1;
            }
        }
