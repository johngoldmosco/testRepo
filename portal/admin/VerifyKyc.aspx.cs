...
public partial class portal_admin_VerifyKyc : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        strQuery = "SELECT a.userid, b.my_sponsar_id, c.username,c.pancard, a.pan_card, c.aadharcard, a.aadhar_card, a.photo, a.cheque, CASE a.kyc_status WHEN 0 THEN 'Pending' END AS KYCStatus, DATE_FORMAT(a.kyc_on,'%d-%b-%Y') AS KYCon FROM mlm_kyc_documents a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c ON a.userid=c.userid WHERE a.kyc_status=0 " + StrSearch + "  ORDER BY a.kyc_on DESC Limit " + intStart + "," + strpageSize + "";
