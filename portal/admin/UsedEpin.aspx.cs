...
public partial class portal_admin_UsedEpin : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid INNER JOIN mlm_login d ON a.used_by=d.userid AND a.status=0 AND a.userid=" + Session["AdminID"] + " " + StrSearch + "");

        strQuery = "SELECT a.userid, b.my_sponsar_id AS User_ID, a.ownerid, c.my_sponsar_id As Owner_ID, a.epin, a.epin_type, a.epin_cost, DATE_FORMAT(a.created_date,'%Y-%b-%d') AS CreatedDate, DATE_FORMAT(a.used_date,'%Y-%b-%d') AS UsedDate, a.used_by, d.my_sponsar_id As UsedByID, a.status FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid INNER JOIN mlm_login d ON a.used_by=d.userid AND a.status=0 AND a.userid=" + Session["AdminID"] + " " + StrSearch + "  Order By a.used_date DESC Limit " + intStart + "," + strpageSize + "";
    ...
    protected string Search()
    {
        ...
        {
            strsel = strsel + " AND DATE(a.used_date) BETWEEN '" + txtStartDate.Text + "' AND '" + txtEndDate.Text + "'";
        }
    ...
    protected void gvMembers_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
    {
        ...
        {
            e.Row.Cells[4].Text = "Package 1";
        }
        if (e.Row.Cells[4].Text == "2")
        {
            e.Row.Cells[4].Text = "Package 2";
        }
        if (e.Row.Cells[4].Text == "3")
        {
            e.Row.Cells[4].Text = "Package 3";
        }
        if (e.Row.Cells[4].Text == "4")
        {
            e.Row.Cells[4].Text = "Package 4";
        }
        if (e.Row.Cells[4].Text == "5")
        {
            e.Row.Cells[4].Text = "Package 5";
        }
        if (e.Row.Cells[4].Text == "6")
        {
            e.Row.Cells[4].Text = "Package 6";
        }
        if (e.Row.Cells[4].Text == "7")
        {
            e.Row.Cells[4].Text = "Package 7";
        }
        if (e.Row.Cells[4].Text == "8")
        {
            e.Row.Cells[4].Text = "Package 8";
        }

        if (e.Row.Cells[9].Text == "0")
        {
            e.Row.Cells[9].Text = "Used";
        }
    }
    ...
    {
        CommonFunctions.exportFile("rptUsedEpin.xls", ".xls", gvMembers, pnllead);
    }
    ...
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.userid, b.my_sponsar_id AS User_ID, a.ownerid, c.my_sponsar_id As Owner_ID, a.epin, a.epin_type, a.epin_cost, DATE_FORMAT(a.created_date,'%Y-%b-%d') AS CreatedDate, DATE_FORMAT(a.used_date,'%Y-%b-%d') AS UsedDate, a.used_by, d.my_sponsar_id As UsedByID, a.status FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid INNER JOIN mlm_login d ON a.used_by=d.userid WHERE a.status=0 AND a.userid=" + Session["AdminID"] + "  Order By a.used_date DESC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    {
        Response.Redirect("UsedEpin.aspx");
    }
}