...
public partial class portal_admin_HolidayManagerr : System.Web.UI.Page
{
    ...
    protected string Search()
    {
        ...
        {
            strsel = strsel + " AND DATE(holiday_date) BETWEEN '" + Convert.ToString(txtStartDate.Text) + "' AND '" + Convert.ToString(txtEndDate.Text) + "' ";
        }
        return strsel;
    }
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_holidays WHERE 1 " + StrSearch + "");

        strQuery = "SELECT id, DATE_FORMAT(holiday_date,'%a %b %e %Y') as holiday_date, status FROM `mlm_holidays` where 1 " + StrSearch + " ORDER BY holiday_date ASC Limit " + intStart + "," + strpageSize + "";
    ...
    {
        CommonFunctions.exportFile("HolidaysManager.xls", ".xls", gvReport, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT id, DATE_FORMAT(holiday_date,'%a %b %e %Y') as holiday_date, status FROM `mlm_holidays` where 1  ORDER BY holiday_date ASC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    protected void lnkActive_Click(object sender, System.EventArgs e)
    {
        ...
        foreach (GridViewRow rw in gvReport.Rows)
        {
            ...
            {
                int intCurStatus = clsOdbc.executeScalar_int("SELECT status FROM mlm_holidays WHERE id=" + vid);

                if (intCurStatus != 1)
                {
                    clsOdbc.executeNonQuery("UPDATE mlm_holidays SET status=1 WHERE id=" + vid + "");
                }
                i = i + 1;
            }
        }
    ...
    protected void lnkDisable_Click(object sender, System.EventArgs e)
    {
        ...
        foreach (GridViewRow rw in gvReport.Rows)
        {
            ...
            if (chkSel.Checked == true)
            {
                ...
                if (intCurStatus != 0)
                {
                    clsOdbc.executeNonQuery("UPDATE mlm_holidays SET status=0 WHERE id=" + vid + "");
                }
                i = i + 1;
            }
        }
