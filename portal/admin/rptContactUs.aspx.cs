...
public partial class portal_admin_rptContactUs : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_contact_us WHERE 1 " + StrSearch + "");

        strQuery = "SELECT id, name, subject,mobile_number, email, message, DATE_FORMAT(created_on,'%Y-%b-%d') AS CreatedON FROM mlm_contact_us  WHERE 1 " + StrSearch + "  Order By created_on DESC Limit " + intStart + "," + strpageSize + "";
  ...
  //              lblError.Text = "Sorry, No Records Found!";
    ...
    {
        Response.Redirect("rptContactUs.aspx");
    }
    protected void lnkbtnExportExcel_Click(object sender, EventArgs e)
    {
        CommonFunctions.exportFile("rptContactUs.xls", ".xls", gvReport, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT name, subject, mobile_number, email, message, DATE_FORMAT(created_on,'%Y-%b-%d') AS CreatedON FROM mlm_contact_us Order By created_on DESC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    {
        Response.Redirect("rptSample.aspx");
    }
