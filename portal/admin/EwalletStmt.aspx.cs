...
public partial class portal_admin_EwalletStmt : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_transaction a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c ON a.userid=c.userid WHERE 1 " + StrSearch + "");

        strQuery = "SELECT a.id, a.trans_number, a.userid, b.my_sponsar_id, c.username, a.debit_amount, a.credit_amount, a.tds, a.ser_charge, a.total_amt, a.closing_balance, DATE_FORMAT(a.trans_date,'%d-%b-%Y') AS TransDate, a.description FROM mlm_transaction a INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_personal_details c ON a.userid=c.userid WHERE 1 " + StrSearch + "  ORDER BY a.id DESC Limit " + intStart + "," + strpageSize + "";
