...
public partial class portal_admin_rptMirrorBonus : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT Count(1) FROM mlm_direct_income a,mlm_personal_details c,mlm_login b,mlm_login d,mlm_personal_details e where  a.child_id = b.userid and a.child_id  = c.userid and d.userid=e.userid and a.userid=d.userid " + StrSearch + " Order By a.userid DESC ");

        strQuery = "SELECT d.my_sponsar_id as USERID,e.UserName as USERNAME,b.my_sponsar_id,c.UserName as child_name,a.amt ,DATE_FORMAT(a.created_on,'%d %M %Y') As recieved_date,a.status FROM mlm_direct_income a,mlm_personal_details c,mlm_login b,mlm_login d,mlm_personal_details e where  a.child_id = b.userid and a.child_id  = c.userid and d.userid=e.userid and a.userid=d.userid " + StrSearch + " Order By a.userid DESC LIMIT " + intStart + "," + strpageSize + "";
    ...
    {
        CommonFunctions.exportFile("reportMirrorBonus.xls", ".xls", gvBinaryIncome, pnllead);
    }
    ...
    {
        Response.Redirect("rptMirrorBonus.aspx");
    }
