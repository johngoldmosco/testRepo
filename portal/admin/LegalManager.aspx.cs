...
public partial class portal_admin_LegalManager : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM tbl_legal WHERE 1 " + StrSearch + "");

        strQuery = "SELECT `id`, `imgUrl`, CASE `status` WHEN 0 THEN 'Hide' WHEN 1 THEN 'Show' END AS Status, DATE_FORMAT(`created_on`,'%d-%b-%Y') AS CreatedOn FROM `tbl_legal` WHERE 1 " + StrSearch + " Limit " + intStart + "," + strpageSize + "";
    ...
    {
        Response.Redirect("LegalManager.aspx");
    }
    protected void lnkbtnExportExcel_Click(object sender, EventArgs e)
    {
        CommonFunctions.exportFile("LegalManager.xls", ".xls", gvReport, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT `id`, `imgUrl`, CASE `status` WHEN 0 THEN 'Hide' WHEN 1 THEN 'Show' END AS Status, DATE_FORMAT(`created_on`,'%d-%b-%Y') AS CreatedOn FROM `tbl_legal` WHERE 1 Order By created_on DESC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    protected void lnkActive_Click(object sender, System.EventArgs e)
    {
        ...
        foreach (GridViewRow rw in gvReport.Rows)
        {
            ...
            {
                int intCurStatus = clsOdbc.executeScalar_int("SELECT status FROM tbl_legal WHERE id=" + vid);

                if (intCurStatus != 1)
                {
                    clsOdbc.executeNonQuery("UPDATE tbl_legal SET status=1 WHERE id=" + vid + "");
                }
                i = i + 1;
            }
        }
    ...
    protected void lnkDisable_Click(object sender, System.EventArgs e)
    {
        ...
        foreach (GridViewRow rw in gvReport.Rows)
        {
            ...
            if (chkSel.Checked == true)
            {
                ...
                if (intCurStatus != 0)
                {
                    clsOdbc.executeNonQuery("UPDATE tbl_legal SET status=0 WHERE id=" + vid + "");
                }
                i = i + 1;
            }
        }
