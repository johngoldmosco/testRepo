...
public partial class portal_admin_rptAward : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_login a INNER JOIN mlm_binary_daily_payout b ON a.userid=b.userid WHERE a.package_id> 1 group by a.userid  " + StrSearch + " ");

        strQuery = "SELECT  a.userid, a.my_sponsar_id, COALESCE(sum(b.match_comit),0) AS Total_Business, CONCAT('Package',' - ', a.package_id) AS Package, CASE a.award_id WHEN 0 THEN 'NONE' WHEN 1 THEN 'Mobile'  WHEN 2 THEN 'Laptop'  WHEN 3 THEN 'Bike'  WHEN 4 THEN 'Royal Enfield'  WHEN 5 THEN 'Alto'  WHEN 6 THEN 'Kwid'  WHEN 7 THEN 'Swift'  WHEN 8 THEN 'Breeza'  WHEN 9 THEN 'Creta' WHEN 10 THEN 'Fortuna' END AS Award FROM mlm_login a INNER JOIN mlm_binary_daily_payout b ON a.userid=b.userid WHERE a.package_id> 1 group by a.userid " + StrSearch + " Order By a.userid ASC";
    ...
    {
        CommonFunctions.exportFile("rptAward.xls", ".xls", gvBinaryIncome, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT userid, CASE award_id WHEN 0 THEN 'NONE' WHEN 1 THEN 'Mobile'  WHEN 2 THEN 'Laptop'  WHEN 3 THEN 'Bike'  WHEN 4 THEN 'Royal Enfield'  WHEN 5 THEN 'Alto'  WHEN 6 THEN 'Kwid'  WHEN 7 THEN 'Swift'  WHEN 8 THEN 'Breeza'  WHEN 9 THEN 'Creta' WHEN 10 THEN 'Fortuna' END AS Award, a.my_sponsar_id FROM `mlm_login` WHERE 1 Order By a.userid ASC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    {
        Response.Redirect("rptAward.aspx");
    }
