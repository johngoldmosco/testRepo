...
public partial class portal_admin_ActiveEpin : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid  WHERE a.status=1 AND a.userid=" + Session["AdminID"] + " " + StrSearch + "");

        strQuery = "SELECT a.userid, b.my_sponsar_id AS User_ID, a.ownerid, c.my_sponsar_id AS Owner_ID, a.epin, a.epin_type, a.epin_cost, DATE_FORMAT(a.created_date,'%Y-%b-%d') AS CreatedDate, a.status FROM mlm_epin a  INNER JOIN mlm_login b ON a.userid=b.userid INNER JOIN mlm_login c ON a.ownerid=c.userid  WHERE a.status=1 " + StrSearch + " AND a.userid="+ Session["AdminID"] +"  ORDER BY a.created_date DESC Limit " + intStart + "," + strpageSize + "";
