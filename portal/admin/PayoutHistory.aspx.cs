...
public partial class portal_admin_PayoutHistory : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_request a INNER JOIN mlm_login b ON a.userid=b.userid WHERE a.status!=0 " + strSel + "");

        strQuery = "SELECT a.id, a.userid, b.my_sponsar_id, a.wallet1, a.request_amount, a.amount_given, a.tds, a.service_charge, a.request_date, a.confirmed_on, a.reject_on, a.status FROM mlm_request a INNER JOIN mlm_login b ON a.userid=b.userid WHERE a.status!=0 " + strSel + " Order By a.request_date DESC  LIMIT " + intStart + "," + strpageSize + "";
    ...
    protected void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
			lblSuccess.Text = clsOdbc.executeScalar_str("SELECT COALESCE(SUM(amount_given),0) FROM mlm_request WHERE status=1");
			lblReject.Text = clsOdbc.executeScalar_str("SELECT COALESCE(SUM(amount_given),0) FROM mlm_request WHERE status=2");
