...
public partial class portal_franchise_FundTransferToFrans : System.Web.UI.Page
{
    ...
    protected string Search()
    {
        ...
        {
            strsel = strsel + " AND DATE(a.transfer_on) BETWEEN '" + txtStartDate.Text + "' AND '" + txtEndDate.Text + "'";
        }
        return strsel;
    }
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_fund_trans_to_franchise a INNER JOIN mlm_franchise b ON a.fransid=b.fransid INNER JOIN mlm_login c ON a.transfer_by=c.userid  WHERE a.fransid='"+ Session["FransID"] +"' " + StrSearch + " ");

        strQuery = "SELECT a.id, a.fransid, b.my_sponsar_id AS FranID, b.name, a.amount, a.transfer_by, c.my_sponsar_id, DATE_FORMAT(a.transfer_on,'%d-%m-%Y') AS DatedOn FROM mlm_fund_trans_to_franchise a INNER JOIN mlm_franchise b ON a.fransid=b.fransid INNER JOIN mlm_login c ON a.transfer_by=c.userid WHERE a.fransid='" + Session["FransID"] + "' " + StrSearch + " Order By a.id DESC";
    ...
    {
        CommonFunctions.exportFile("FundReceivedByFrans.xls", ".xls", gvBinaryIncome, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.id, a.fransid, b.my_sponsar_id AS FranID, b.name, a.amount, a.transfer_by, c.my_sponsar_id, DATE_FORMAT(a.transfer_on,'%d-%m-%Y') AS DatedOn FROM mlm_fund_trans_to_franchise a INNER JOIN mlm_franchise b ON a.fransid=b.fransid INNER JOIN mlm_login c ON a.transfer_by=c.userid WHERE a.fransid='" + Session["FransID"] + "' Order By a.id DESC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    {
        Response.Redirect("FundReceivedByFrans.aspx");
    }
