...
public partial class portal_franchise_rptRepurchase : System.Web.UI.Page
{
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM mlm_repurchase a INNER JOIN mlm_login b ON a.userid = b.userid INNER JOIN mlm_login c ON a.franchise_id = c.userid INNER JOIN mlm_personal_details d ON a.userid=d.userid WHERE a.franchise_id=" + Session["FransID"] + " " + StrSearch + " ");

        strQuery = "SELECT a.id, a.userid, b.my_sponsar_id AS User_ID, d.username, a.franchise_id, c.my_sponsar_id AS FransID, a.total_amount, a.total_bv, a.total_tax, a.gross_amount, DATE_FORMAT(a.repurchase_on,'%Y-%m-%d') AS RepurchaseDate FROM mlm_repurchase a INNER JOIN mlm_login b ON a.userid = b.userid INNER JOIN mlm_login c ON a.franchise_id = c.userid INNER JOIN mlm_personal_details d ON a.userid=d.userid WHERE a.franchise_id='" + Session["FransID"] + "' " + StrSearch + " Order By a.id DESC";
    ...
    {
        CommonFunctions.exportFile("rptRepurchase.xls", ".xls", gvBinaryIncome, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.id, a.userid, b.my_sponsar_id AS User_ID, d.username, a.franchise_id, c.my_sponsar_id AS FransID, a.total_amount, a.total_bv, a.total_tax, a.gross_amount, DATE_FORMAT(a.repurchase_on,'%Y-%m-%d') AS RepurchaseDate FROM mlm_repurchase a INNER JOIN mlm_login b ON a.userid = b.userid INNER JOIN mlm_franchise c ON a.franchise_id = c.fransid INNER JOIN mlm_personal_details d ON a.userid=d.userid WHERE a.franchise_id= '" + Session["FransID"] + "'  Order By a.id DESC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    protected void gvBinaryIncome_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        foreach (GridViewRow rw in gvBinaryIncome.Rows)
        {
            ...
            string strID = null;
            strID = rw.Cells[1].Text;

            lnkActivate = (HyperLink)rw.FindControl("lnkUpdate");

            lnkActivate.NavigateUrl = "RepurchaseInvoice.aspx?ID=" + strID;
