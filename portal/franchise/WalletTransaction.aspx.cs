...
public partial class portal_franchise_WalletTransaction : System.Web.UI.Page
{
    ...
    protected string Search()
    {
        ...
        {
            strsel = strsel + " AND DATE(a.trans_date) BETWEEN '" + txtStartDate.Text + "' AND '" + txtEndDate.Text + "'";
        }
        return strsel;
    }
    ...
    private DataView GetData(int intpageindex)
    {
        ...
        int count = clsOdbc.executeScalar_int("SELECT COUNT(1) FROM `mlm_franchise_transaction` a INNER JOIN mlm_franchise b ON a.fransid=b.fransid LEFT JOIN mlm_login c ON a.userid=c.userid WHERE a.fransid='" + Session["FransID"] + "' " + StrSearch + " ");

        strQuery = "SELECT a.id,a.fransid,b.my_sponsar_id AS FranID, b.name,  a.userid,c.my_sponsar_id As UserSPID, a.debit_amount, a.credit_amount, CASE a.trans_type WHEN 1 THEN 'Add Money' WHEN 2 THEN  'Repurchase' END AS TransType, a.current_amt, DATE_FORMAT(a.trans_date,'%Y-%b-%d') AS TransDate, a.description FROM `mlm_franchise_transaction` a INNER JOIN mlm_franchise b ON a.fransid=b.fransid LEFT JOIN mlm_login c ON a.userid=c.userid WHERE a.fransid='" + Session["FransID"] + "' " + StrSearch + " Order By a.id DESC";
    ...
    {
        CommonFunctions.exportFile("WalletTransaction.xls", ".xls", gvBinaryIncome, pnllead);
    }
    protected void lnkPrint_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string strQuery = "SELECT a.id,a.fransid,b.my_sponsar_id, b.name,  a.userid,c.my_sponsar_id, a.debit_amount, a.credit_amount, CASE a.trans_type WHEN 1 THEN 'Add Money' WHEN 2 THEN  'Repurchase' END AS TransType, a.current_amt, DATE_FORMAT(a.trans_date,'%Y-%b-%d') AS TransDate, a.description FROM `mlm_franchise_transaction` a INNER JOIN mlm_franchise b ON a.fransid=b.fransid LEFT JOIN mlm_login c ON a.userid=c.userid WHERE a.fransid='" + Session["FransID"] + "' Order By a.id DESC";
            ds = clsOdbc.getDataSet(strQuery);
    ...
    {
        Response.Redirect("WalletTransaction.aspx");
    }
